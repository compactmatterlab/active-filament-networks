...
FAct = (NumMyo / NumAct * FPerMyo) if RatioAct != 0 else 0
    ...
    if State[i] == 1 and AvgKinPerMT != 0:  # MT
        ...
        for j in range(NumNB3):
            if State[NB3[i, j]] == 2 and TrackerFM3[NB3[i, j], NB3Partner[j]] == 0:  # actin
                FMag3[i, NB3[i, j]], FMag3[NB3[i, j], i] = FDir3[i, NB3[i, j]] * FAct, FDir3[NB3[i, j], i] * FAct
                TrackerFM3[i, j], TrackerFM3[NB3[i, j], NB3Partner[j]] = 1, 1
        for j in range(NumNB4):
            if State[NB4[i, j]] == 2 and TrackerFM4[NB4[i, j], NB4Partner[j]] == 0:  # actin
                FMag4[i, NB4[i, j]], FMag4[NB4[i, j], i] = FDir4[i, NB4[i, j]] * FAct, FDir4[NB4[i, j], i] * FAct
                TrackerFM4[i, j], TrackerFM4[NB4[i, j], NB4Partner[j]] = 1, 1
...
for i in range(TotPts):
    if (State[i] == 1 and AvgKinPerMT != 0) or (State[i] == 2 and FAct != 0):  # MT or actin
        for j in range(NumNB1):
...
NPasKin, NActKinTot = np.zeros(TotPts), np.zeros(TotPts)
...
for i in range(TotPts):
    ...
    if State[i] == 1:  # MT
        ...
        for j in range(NumNB1):
            ...
            FrictionAct = 1 + (GamMotMyo * MyoPerAct * SameNB[i]) / GamFilAct + (GammaCLAct * SameNB[i]) / GamFilAct
            # VDtDel = FDtDel / (GammaFil + GammaMot*N + GammaCL*N)
            if FDtDel[i, j] > 0:
                VDtDelAct = FDtDel[i, j] / (GamFilAct + GamMotMyo * MyoPerAct * SameNB[i] + GammaCLAct * SameNB[i])
            else:
                VDtDelAct = 0
            # K = k0/friction + VDtDel / s
...
while Tau < (TotalTime * 60):
    ...
    if DoSomethingMoveID == 1:
            ...
            if State[PtMv[i]] == 1 and AvgKinPerMT != 0:  # MT
                ...
                for j in range(NumNB3):
                    if State[NB3[PtMv[i], j]] == 2 and TrackerFM3[NB3[PtMv[i], j], NB3Partner[j]] == 0:  # actin
                        FMag3[PtMv[i], NB3[PtMv[i], j]] = FDir3[PtMv[i], NB3[PtMv[i], j]] * FAct
                        FMag3[NB3[PtMv[i], j], PtMv[i]] = FDir3[NB3[PtMv[i], j], PtMv[i]] * FAct
                        TrackerFM3[PtMv[i], j], TrackerFM3[NB3[PtMv[i], j], NB3Partner[j]] = 1, 1
                for j in range(NumNB4):
                    if State[NB4[PtMv[i], j]] == 2 and TrackerFM4[NB4[PtMv[i], j], NB4Partner[j]] == 0:  # actin
                        FMag4[PtMv[i], NB4[PtMv[i], j]] = FDir4[PtMv[i], NB4[PtMv[i], j]] * FAct
                        FMag4[NB4[PtMv[i], j], PtMv[i]] = FDir4[NB4[PtMv[i], j], PtMv[i]] * FAct
                        TrackerFM4[PtMv[i], j], TrackerFM4[NB4[PtMv[i], j], NB4Partner[j]] = 1, 1
        ...
        for i in range(len(PtAf)):
            if (State[PtAf[i]] == 1 and AvgKinPerMT != 0) or (State[PtAf[i]] == 2 and FAct != 0):  # MT or actin
                for j in range(NumNB1):
        ...
        for i in range(len(PtAf)):
            ...
            if State[PtAf[i]] == 1:  # MT
                ...
                for j in range(NumNB1):
                    # Friction = 1 + (GammaMot*N)/GammaFil + (GammaCL*N)/GammaFil
                    FrictionAct = 1 + (GamMotMyo * MyoPerAct * SameNB[PtAf[i]]) / GamFilAct + (
                                GammaCLAct * SameNB[PtAf[i]]) / GamFilAct
                    # VDtDel = FDtDel / (GammaFil + GammaMot*N + GammaCL*N)
                    if FDtDel[PtAf[i], j] > 0:
                        VDtDelAct = FDtDel[PtAf[i], j] / (
                                GamFilAct + GamMotMyo * MyoPerAct * SameNB[PtAf[i]] + GammaCLAct * SameNB[PtAf[i]])
                        # print(VDtDelAct)
                    else:
                        VDtDelAct = 0
                    # K = k0/friction + VDtDel / s
...
for i in range(TotPts):
    if (State[i] == 1 and AvgKinPerMT != 0) or (State[i] == 2 and FAct != 0):
