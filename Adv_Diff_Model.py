...
KinesinPerMT, MyosinPerActin = 100, 0  # - / -
...
TotalTime, TimeStep = 10 * 60 * 60 * 1000, 250  # ms / ms
ActBound, ActBoundPct = True, 2  # - / %
...
MaxActMoves, NumActMoves, ActLimit = (ActBoundPct / 100) * NumActin, 0, 0
...
while ElapsedTime < TotalTime:
    ...
    if (State[PtMoveA] == 2 and State[PtMoveB] == 3) or (State[PtMoveA] == 3 and State[PtMoveB] == 2):
        NumActMoves += 1
    ...
    # Reset probabilities if boundary parameter is reached
    if NumActMoves > MaxActMoves:
        CombProb[np.where(State == 2), :] = 0
        CombProb[:, np.where(State == 2)] = 0
    ...
    if TrackerTS == 0:
        ...
        if ActBound is True and NumActMoves > MaxActMoves and ActLimit == 0:
            ActLimit = 1
            for i in range(TotPts):
                if State[i] == 2 or State[i] == 3:
                    ProbAllMoveIDs[i, 0] = CombProb[i, NB1[i, 1]]
                    ProbAllMoveIDs[i, 1] = CombProb[i, NB1[i, 0]]
                    ProbAllMoveIDs[i, 2] = CombProb[i, NB1[i, 5]]
    elif TrackerTS == 1:
        NumActMoves, ActLimit = 0, 0
        ...
        # Update elapsed time
        ElapsedTime += TimeStep
    # Increment timestep if sum of movement probabilities is zero but actin boundary is reached
    while np.sum(ProbAllMoveIDs) == 0:
        if ActBound is True and NumActMoves > MaxActMoves:
            # Update force magnitude and direction matrices
            for i in range(len(PtMv)):  # reset entries
                FMag1[PtMv[i], NB1[PtMv[i]]], FMag1[NB1[PtMv[i]], PtMv[i]] = 0, 0
                FDir1[PtMv[i], NB1[PtMv[i]]], FDir1[NB1[PtMv[i]], PtMv[i]] = 1, 1
